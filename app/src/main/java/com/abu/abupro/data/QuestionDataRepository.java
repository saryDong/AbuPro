...
public class QuestionDataRepository implements QuestionDataSource {
    ...
    @Override
    public void getRecentQuestionList(final LoadCallback<Question> callback,int list_type) {
        getRecentQuestionListFromServer(callback, false,list_type);
    }

    private void getRecentQuestionListFromServer(final LoadCallback<Question> callback, final boolean isLoadMore,final int list_type) {
        //获取查询对象
        final AVQuery<Question> questionAVQuery = AVObject.getQuery(Question.class);
        questionAVQuery.whereEqualTo(Question.LIST_TYPE,list_type);
    ...
    @Override
    public void getMoreRecentQuestionList(final LoadCallback<Question> callback,int list_type) {
        getRecentQuestionListFromServer(callback, true,list_type);
    }

    @Override
    public void getHotQuestionList(final LoadCallback<Question> callback,int list_type) {
        getHotQuestionListFromServer(callback, false,list_type);
    }

    private void getHotQuestionListFromServer(final LoadCallback<Question> callback, final boolean isLoadMore,int list_type) {
        final AVQuery<Question> questionAVQuery = AVObject.getQuery(Question.class);
        questionAVQuery.whereEqualTo(Question.LIST_TYPE,list_type);
    ...
    @Override
    public void getMoreHotQuestionList(final LoadCallback<Question> callback,int list_type) {
        getHotQuestionListFromServer(callback, true,list_type);
    }
    ...
    @Override
    public void addQuestion(String title, String des,int addQuestion, final SaveCallback callback) {
        Question question = new Question();
        question.setTitle(title);
        question.setDescription(des);
        question.setListType(addQuestion);
